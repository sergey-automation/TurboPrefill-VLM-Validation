...
def load_cfg(path=CONFIG_PATH):
    cfg = {}
    text = (SCRIPT_DIR / path).read_text(encoding="utf-8")
    ...
    defaults = {
        ...
        "GGML_SCHED_DEBUG": "0",
        "GGML_CUDA_DEBUG": "0",
        "LLAMA_SERVER_BIN": str(SCRIPT_DIR / "build/bin/llama-server"),
        "LOCAL_LD_LIBRARY_PATH": str(SCRIPT_DIR / "build/bin"),
        "UNSET_LD_PRELOAD": "1",
    }
...
def start_server(out_dir):
    ...
    tp_mode = os.environ.get("TURBOPREFILL", cfg.get("TURBOPREFILL", "0")).strip()
    server_bin = str(Path(cfg["LLAMA_SERVER_BIN"]).expanduser())
    local_ld_library_path = cfg.get("LOCAL_LD_LIBRARY_PATH", str(SCRIPT_DIR / "build/bin"))
    local_ld_library_path = str(Path(local_ld_library_path).expanduser())

    log.write(f"TURBOPREFILL      : {tp_mode}\n")
    log.write(f"MODEL             : {cfg['MODEL']}\n")
    log.write(f"CTX_SIZE          : {cfg['CTX_SIZE']}\n")
    log.write(f"BATCH             : {cfg['BATCH']}\n")
    log.write(f"UBATCH            : {cfg['UBATCH']}\n")
    log.write(f"LLAMA_SERVER_BIN  : {server_bin}\n")
    log.write(f"LOCAL_LIB_DIR     : {local_ld_library_path}\n")
    log.flush()
    ...
    env["LD_LIBRARY_PATH"] = local_ld_library_path + (
        ":" + env["LD_LIBRARY_PATH"] if env.get("LD_LIBRARY_PATH") else ""
    )

    if cfg.get("UNSET_LD_PRELOAD", "1") != "0":
...
def collect_environment():
    info = {
        ...
        "LLAMA_SERVER_BIN": cfg.get("LLAMA_SERVER_BIN", ""),
...
def write_report(report, raw_csv, results, server_log, server_cmd, env_info):
    ...
    with open(report, "w", encoding="utf-8") as f:
        ...
        f.write("## Test header\n\n")
        for key in ["MODEL", "NGL", "CTX_SIZE", "N_GEN", "BATCH", "UBATCH", "CTK", "SPEC_TYPE", "SPEC_DRAFT_N_MAX", "SPLIT_MODE", "TENSOR_SPLIT", "PARALLEL", "TEMPERATURE"]:
